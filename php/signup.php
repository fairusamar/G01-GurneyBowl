...
if(isset($_POST['fname']) && 
   ...
   isset($_POST['pass'])){
    ...
    if (empty($fname)) {
    	...
    	header("Location: ../ADDAd.php?error=$em&$data");
	    exit;
    }else if(empty($uname)){
      ...
      if (isset($_FILES['pp']['name']) AND !empty($_FILES['pp']['name'])) {
         ...
         if($error === 0){
            ...
            if(in_array($img_ex_to_lc, $allowed_exs)){
               ...
               header("Location: ../ADDAd.php?success=New account has been created successfully");
                exit;
            }else {
	...
	header("Location: ../ADDAd.php?error=error");
	exit;
}
